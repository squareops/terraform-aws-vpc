locals {
  ...
  database_subnets_native = var.vpc_database_subnet_enabled ? length(var.vpc_database_subnet_cidrs) > 0 ? var.vpc_database_subnet_cidrs : [for netnum in range(local.azs * 2, local.azs * 3) : cidrsubnet(var.vpc_cidr, 8, netnum)] : []
...
module "vpc" {
  ...
  create_database_subnet_group                    = length(local.generate_subnets.vpc_database_subnets) > 1 && var.database_subnet_group_enabled ? true : false
  intra_subnets                                   = local.vpc_intra_subnets
  public_subnets                                  = local.vpc_public_subnets
  private_subnets                                 = local.vpc_private_subnets
  database_subnets                                = local.database_subnets
  enable_flow_log                                 = var.vpc_flow_log_enabled
  enable_nat_gateway                              = length(local.generate_subnets.vpc_private_subnets) > 0 && !var.ipv6_only ? true : false
  ...
  default_security_group_egress                   = vpc_vpc_default_security_group_egress
...
resource "aws_vpc_endpoint" "private-s3" {
  ...
  vpc_endpoint_type = var.vpc_endpoint_type_private-s3
  policy            = <<POLICY
{
    "Statement": [
        {
...
resource "aws_vpc_endpoint" "private-ecr-dkr" {
  ...
  vpc_endpoint_type   = var.vpc_endpoint_type_ecr-dkr
...
resource "aws_vpc_endpoint" "private-ecr-api" {
  ...
  vpc_endpoint_type   = var.vpc_endpoint_type_ecr-api
  private_dns_enabled = true
  policy              = <<POLICY
{
    "Statement": [
